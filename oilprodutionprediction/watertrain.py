...
from oilprodutionprediction.ModelEvaluation import ModelEvaluation
from oilprodutionprediction.BuildModel import BuildModel
from oilprodutionprediction.CreateDataSet import CreateDataSet
...
def rm_main(data_frame1,data_frame2,):
    ...
    if diff:
        ...
        data_frame2 = one_well_diff(data_frame2, False)
    ...
    # 生成测试样本
    x_test, y_test, jh_test, rq_test = model_act.CreatTestData(test_out_value_arr, test_out_jh, test_out_rq)
    '''
    # 生成样本
    ...
    #model_act.MyGridSearchCV(x_train, y_train)
    ...
    # 训练模型
    model_act.FitModel(model, x_train, y_train,x_test,y_test)
    # 保存模型文件
    model_act.SaveModel(model, modelpath)
    # 预测结果
    y_predict_unit = model.predict(x_test)
    # 预测结果保存
    y_pre = []
    y_te = []
    for i in range(len(y_predict_unit)):
        y_pre.append(y_predict_unit[i][0])
        y_te.append(y_test[i][0])
...
def one_well_diff(data,delete):
    ...
    for well_name in well_names:
        child_data = data[data["JH"] == well_name]
...
if __name__ == "__main__":
    ...
    #data_csv = "特高3个区块.csv"
    data_csv = "高含水样本2.csv"
    data_train_csv = "high-train - water.csv"
    data_test_csv = "high-test - water.csv"
    #data_train_csv = "高含水样本2 - train.csv"
    #data_test_csv = "高含水样本2 - test.csv"
    modelpath = "E:\study\projects\oilproductionprediction\datafiles\multiwells"
    ...
    path_train_result = modelpath + "/" +"1含水率预测结果-LSTM.csv"
    pre_result.to_csv(path_train_result, encoding="utf_8_sig", index=False)
    path_estimate =modelpath + "/" + "1含水率误差比较-LSTM.csv"
